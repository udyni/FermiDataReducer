...
"""
import numpy as np
from DataReducerOffline import DataReducerOffline
...
# ================
# SAVE PATH
# Path were to save reduced data. If save path is 'None', by default reduced data is saved in the 'work' directory of each run.
save_path = None


# ================
# SAVE SUFFIX
# Suffix of the saved reduced file, that will be Run_<number>_<suffix>.h5
save_suffix = 'offline'
...
# CONFIGURATION
# Configure log file
logfile = '/tmp/reducer-offline.log'

# Number of workers: number of parallel processes that process acquisition runs
number_of_workers = 4

# Runs to reduce: should be a list of run numbers. You can use ... between two run numbers to include all the runs
# between the two. There's no limit to the number of runs
# E.G. runs_to_reduce = [234, 236, ..., 250, 255, ..., 260]
runs_to_reduce = [...]
...
dr = DataReducerOffline(options, remote_path, save_path=save_path, save_suffix=save_suffix, nworkers=number_of_workers, log_level=Logger.DEBUG if DEBUG else Logger.INFO)
dr.addLogFile(logfile)
dr.run(runs_to_reduce)
